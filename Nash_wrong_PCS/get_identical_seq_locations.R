...
speciesDir<-paste0("/bucket/MillerU/Abrar/Nash_wrong_PCS/hg19-gorGor3/", chrom, "/")
outDir<-paste0("/bucket/MillerU/Abrar/Nash_wrong_PCS/hg19-gorGor3/", chrom, "/")
...
if(species2 != "lepOcu1"){
	axts<-lapply(species2, function(x) {
		     lel<-dir(paste0("/bucket/MillerU/Abrar/axtNet/hg19gorGor3/"), pattern=paste0(x, ".*.axt"), full.names=T)
		     lel<-lel[!grepl("Exon", lel)]
		     lel<-lel[!grepl("broken", lel)]
         print(lel)
		     tfn<-paste0("/bucket/MillerU/Abrar/bigZips/hg19.2bit") #target sequence: Human
		     if(!file.exists(tfn)) tfn<-paste0("/bucket/MillerU/Abrar/bigZips/hg19.2bit")
		     qfn<-paste0("/bucket/MillerU/Abrar/bigZips/gorGor3.2bit") #query sequence: Others
		     if(!file.exists(qfn)) qfn<-paste0("/bucket/MillerU/Abrar/bigZips/gorGor3.2bit")
