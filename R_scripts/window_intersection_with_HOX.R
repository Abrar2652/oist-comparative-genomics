...
for(window_size in window_list)
{
      for(percent in percent_list)
      {
            ...
            {   
                ...
                identical_seqs.gr<-makeGRangesFromDataFrame(read_excel("/bucket/.mabuya/MillerU/Abrar/PCS/hg38_homebox_genes_coordinates.xlsx"))
                ...
                #print(identical_seqs.gr)
                print(tiled_chr)
                write.table( x = data.frame(tileHits), file = paste0("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-gorGor6/HOX_intersected_",chrom,".csv"), sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE )
            }
      }
}
