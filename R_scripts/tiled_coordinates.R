...
window_size<-as.numeric(50000) 	##in bp  (10000000,5000000,2500000,1200000,500000,100000,50000)

species1<-'hg38'			## UCSC stle abbreviation (hg38, micMur3, etc)	
species2<-list('ponAbe3'=1,'nomLeu3'=1,'calJac4'=1,'saiBol1'=1,'tarSyr2'=1,'micMur3'=1,'galVar1'=1,'oryCun2'=1,'felCat9'=1,'equCab3'=1,'susScr11'=1,'oviAri4'=1,'triMan1'=1,'ornAna2'=1,'panPan3'=1,'panTro6'=1)
...
#chrom_list=list('chr1')

for(i in seq_along(species2))
{
    outDir<-paste0("/bucket/MillerU/Abrar/hg38-", names(species2)[i], "_outputs/hg38-", names(species2)[i], "_tiled_coordinates_", window_size/1000, "kb/")
    ...
    {    
        ...
        pcs_by_tile<-lapply(as.list(1:length(tiled_chr)) , function(x){
                    ...
                    if(is.null(obj)){
                      ...
                      score = unlist(lapply(score, as.tuple),recursive = F)
                      return(score)
                    }
                    })
        ...
        writeLines(dt_text, paste0(outDir, chrom, "_", window_size/1000, "kb_min_window_tiled_coordinates.txt"))
        print("tiled coordinates calculated")
    }
}
#gc()