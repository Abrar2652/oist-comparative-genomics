...
for genome in genome_list:
    ...
    path = '/bucket/.mabuya/MillerU/Abrar/PCS/AR/indel_terminated/AR_only_hg_lowercase_ignore_{0}'.format(genome)
    os.makedirs(path)
    # saving the dataframe
    #df.to_csv(r'/bucket/MillerU/Abrar/PCS/hg38-{0}/indel_terminated_PCS_hg38{0}.csv'.format(genome), index = False) 
    df.to_csv(r'/bucket/.mabuya/MillerU/Abrar/PCS/AR/indel_terminated/AR_only_hg_lowercase_ignore_{0}/indel_terminated_AR_hg38{0}_hg_lowercase_ignore_{0}.csv'.format(genome), index = False) 
