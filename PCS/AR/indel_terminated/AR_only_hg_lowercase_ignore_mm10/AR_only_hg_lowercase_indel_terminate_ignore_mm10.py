...
for genome in genome_list:
    ...
    def compare_strings(df, column, j):
        ...
        for i in range(maxlen):
          ...
          if (letter1 not in ['a','t','c','g']): # uppercase terminate
              result1+='\n'
              #result2+='\n'
          if ((letter1!='-') and letter1 in ['a','t','c','g']):
              result1+=letter1
              #result2+=letter2
          if ((letter1=='-')): # mismatch, indel terminate
              result1+='\n'
              #result2+='\n'
    ...
    df.to_csv(r'/bucket/.mabuya/MillerU/Abrar/PCS/AR/indel_terminated/AR_only_hg_lowercase_ignore_mm10/indel_terminated_AR_hg19{0}_hg_lowercase_ignore_{0}.csv'.format(genome), index = False) 
