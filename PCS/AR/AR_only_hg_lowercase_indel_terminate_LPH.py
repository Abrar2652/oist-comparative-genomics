...
for genome in genome_list:
    ...
    #os.makedirs(path) 
    print(header_frame.head())
    #header_frame.to_csv(r'/bucket/.mabuya/MillerU/Abrar/PCS/AR/indel_terminated/AR_only_hg_lowercase_match/indel_terminated_AR_hg38{0}_hg_lowercase_only.tsv'.format(genome), sep="\t", index=False)
    header_frame.to_csv(r'indel_terminated_AR_hg38{0}_hg_lowercase_only.tsv'.format(genome), sep="\t", index=False)
    ...
    df.to_csv(r'indel_terminated_AR_hg38{0}_hg_lowercase_only.csv'.format(genome), index = False) 
