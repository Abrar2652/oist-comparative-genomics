...
for genome,name in zip(genome_list, names_list):
    ...
    df1 = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/idea2/PCS_common_intersection_primate_L20_10_genomes.csv") #repetitive+vanilla PCS
    df1 = df1[df1['Number of PCS of Length L']!=0]
    df2 = pd.read_csv("/bucket/MillerU/Abrar/PCS/hg38-{0}/PCS_hg38{0}.csv".format(genome)) #vanilla PCS
    
    # Preparing the data for the plot
    x1 = [i for i in df1.index][:]
    x1_fake = [i for i in df1.index][:165]
    x1 = np.array(x1)
    x1_fake = np.array(x1_fake)
    y1 = df1['Number of PCS of Length L'][:]
    y1 = np.array(y1)
    y1_fake = df1['Number of PCS of Length L'][:165]
    ...
    ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared), (300,10**2), fontsize=15, xytext=(300,10**2))
    plt.legend(title="Pairwise Primates Used:\n Human/Gorilla, Human/Orangutan, Human/Rhesus,\n Human/Gibbon, Human/Bonobo, Human/Chimpanzee,\n Human/Baboon, Human/Green Monkey, Human/Proboscis Monkey,\n Human/Crab-eating Macaque", title_fontsize='large', fontsize=13, markerscale=3, loc='upper right')
    
    plt.savefig('/bucket/MillerU/Abrar/PCS/common_primates_pcs_vs_hg38{0}_loglog_10_genomes.png'.format(genome))
