...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-micMur3/PCS_hg38micMur3.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][1:]
x_fake = [i for i in df.index][19:530]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][1:]
y_fake = df['Number of PCS of Length L'][19:530]
x=np.array(x)
...
y_fit = linear(x_fake, 4, 10**10.9) # 10.9 micMur
...
ax.set_title('PCS length distribution of hg38/micMur3 genome alignment for L>=1', fontsize=15)
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-micMur3/hgmicMur3.png")
#plt.show()