...
import pandas as pd
knee_df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/10MB_chrY_idx1_sampled_pcs_coordinates_hg38gorGor6.csv") # knee region coordinates of Gorilla csv file
...
lines=[]
header=[]
for i in ['Y']: #All_files_merged for whole genome PCS calculation
    with open('/bucket/MillerU/Abrar/axtNet/hg38gorGor6/chr{0}.hg38.gorGor6.net.axt'.format(i)) as file:
        while True: 
...
header_frame.merge(knee_df,
          left_on=['first.seqnames', 'first.start'],
          right_on=['Chrom', 'Start'],
          how='inner'
         )[header_frame.columns].to_csv(r'/bucket/MillerU/Abrar/10MB_chrY_idx1_sampled_pcs_sequences_hg38gorGor6.csv', index=False)
