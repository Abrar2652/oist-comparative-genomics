...
genome_list = ['aotNan1','cebCap1','cerAty1','manLeu1','macNem1','colAng1','rhiBie1','rhiRox1']
for genome in genome_list:
    ...
    def matched_indices(df, column, j):
        ...
        for i,(letter1, letter2) in enumerate(zip(str1,str2)):#i=index, letter1=str1, letter2=str2
            if ((letter1 == letter2) and (letter1 in ['A','T','C','G']) and (letter2 in ['A','T','C','G'])):  # vanilla pcs
            #if ((letter1.upper() == letter2.upper()) and letter1.upper() in ['A','T','C','G'] and letter2.upper() in ['A','T','C','G']): #with repetitive sequences          
    ...
    chrom = 'all'
    path = '/bucket/MillerU/Abrar/hg38-{1}_outputs/pcs_sequences/{0}'.format(chrom, genome)
    os.makedirs(path) 
    header_frame.to_csv(r'/bucket/MillerU/Abrar/hg38-{1}_outputs/pcs_sequences/{0}/hg38_{1}_{0}_pcs_seqs.tsv'.format(chrom, genome), sep="\t", index=False)
