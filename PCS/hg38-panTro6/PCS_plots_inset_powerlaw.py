...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-panTro6/PCS_hg38panTro6.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][:]
x_fake = [i for i in df.index][499:1260]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][:]
y_fake = df['Number of PCS of Length L'][499:1260]
x=np.array(x)
...
y_fit = linear(x_fake, 6.2, 10**19.4)
...
ax.set_title('PCS length distribution of hg38/panTro6 genome alignment for L>=1', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(1.5,10**4.4), fontsize=15, xytext=(1.5,10**4.4))
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-panTro6/hgpantro6_powerlaw.png")
#plt.show()
