...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-saiBol1/PCS_hg38saiBol1.csv") 
df = df[df['Number of PCS of Length L']!=0][9:]
# Preparing the data for the plot
x = [i for i in df.index][:]
y = df['Number of PCS of Length L'][:]
x_fake = [i for i in df.index][:590]
print(x_fake[0],x_fake[-1])
y_fake = df['Number of PCS of Length L'][:590]#29:790
...
axIns = ax.inset_axes([0.53, 0.51, 0.46, 0.47])
...
ax.set_title('PCS length distribution of hg38/saiBol1 genome alignment for L>=10', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(15,10**4), fontsize=15, xytext=(15,10**4))
ax.set_xlabel('Length (log10)', fontsize=20)
ax.set_ylabel('Number of L-mers (log10)', fontsize=20)
ax.tick_params(axis='x', labelsize=15)
ax.tick_params(axis='y', labelsize=15)
ax.legend(fontsize=15, markerscale=3, loc='lower left')

plt.savefig('/bucket/.mabuya/MillerU/Abrar/PCS/hg38-saiBol1/hgsb1.png')
#ax.show()