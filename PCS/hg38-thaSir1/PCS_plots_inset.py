...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-thaSir1/PCS_hg38thaSir1.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][1:]
x_fake = [i for i in df.index][7:230]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][1:]
y_fake = df['Number of PCS of Length L'][7:230]
x=np.array(x)
...
y_fit = linear(x_fake, 3.7, 10**8.8)
...
ax.set_title('PCS length distribution of hg38/thaSir1 genome alignment for L>=1', fontsize=15)
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-thaSir1/hgthaSir1.png")
#plt.show()
