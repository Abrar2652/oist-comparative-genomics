...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-mm39/PCS_hg38mm39.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][1:]
x_fake = [i for i in df.index][9:355]
#15:510 Lemur
#15:370 Mouse
print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][1:]
y_fake = df['Number of PCS of Length L'][9:355]
x=np.array(x)
...
y_fit = linear(x_fake, 4, 10**10.3)
#10**10.3 mm6, mm10, mm39
#10**10.9 micmur1
...
axIns = ax.inset_axes([0.535, 0.53, 0.46, 0.46]) #.49, 0.48, 0.5, 0.5
...
ax.set_title('PCS length distribution of hg38/mm39 genome alignment for L>=1', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(2*10**2,10**4), fontsize=15, xytext=(3,1000))
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-mm39/hgmm39.png")
#plt.show()