...
df = df[df['Number of PCS of Length L']!=0][11:]
# Preparing the data for the plot
x = [i for i in df.index][:]
y = df['Number of PCS of Length L'][:]
x_fake = [i for i in df.index][:1210]
print(x_fake[0],x_fake[-1])
y_fake = df['Number of PCS of Length L'][:1210]
...
ax.set_title('PCS length distribution of hg38/panPan3 genome alignment for L>=12', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(1.5*10**1,10**4), fontsize=15, xytext = (15,10**4))
...
plt.savefig("/bucket/MillerU/Abrar/PCS/hg38-panPan3/hgpanpan3.png")
#ax.show()