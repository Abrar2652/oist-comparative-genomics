...
for genome in genome_list:
    ...
    os.makedirs(path) 
    header_frame.to_csv(r'/bucket/MillerU/Abrar/hg38-{1}_outputs/get_identical_seq_loc_hg38-{1}/{0}/hg38_{1}_{0}_identical_seqs.tsv'.format(chrom, genome), sep="\t", index=False)
    
    
    print(len(seq_count))
    
    '''
    for index, row in header_frame.iterrows():
        if (((row['first.end'] - row['first.start'] + 1) != seq_count[index]) or ((row['second.end'] - row['second.start'] + 1) != seq_count[index])):
                print("Error")
    print("OK")
    '''
    ...
    df.to_csv(r'/bucket/MillerU/Abrar/PCS/hg38-{0}/PCS_hg38{0}.csv'.format(genome), index = False) 
