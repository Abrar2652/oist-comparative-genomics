...
for i in backtrack_index: #range(len(lines1)):
    #print(i)    
    if(lines1[i] != [[0]]):
        chrom=[]
        ...
        print(my_df.start.iloc[0],my_df.end.iloc[-1])
        my_df.to_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-gorGor6/all_window_comparisons/special/chr1_idx{1}_{0}kb_special_coordinates_hg38gorGor6.csv".format(window_size, i), index=False)
    else:
        continue
