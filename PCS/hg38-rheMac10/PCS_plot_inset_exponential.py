...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-rheMac10/PCS_hg38rheMac10.csv") 
df = df[df['Number of PCS of Length L']!=0][19:]
# Preparing the data for the plot
x = [i for i in df.index][:]
y = df['Number of PCS of Length L'][:]
x_fake = [i for i in df.index][:710]
print(x_fake[0],x_fake[-1])
y_fake = df['Number of PCS of Length L'][:710]#29:790
...
axIns = ax.inset_axes([0.055, 0.055, 0.45, 0.45])
axIns.semilogy(x , y, '*y', linewidth=2, label='Semi-Log')
axIns.semilogy(x_fake, y_fit, 'b--', label="Fitted Curve:\nDegree = 4,\nLower L cutoff = 20")
axIns.legend(fontsize=13, markerscale=2, loc='upper right')

ax.loglog(x, y, '.r', linewidth=2, label='Log-Log')
ax.loglog(x_fake, y_fit, 'b--', label="Fitted Curve:\nDegree = 4,\nLower L cutoff = 20")

ax.set_title('PCS length distribution of hg38/rheMac10 genome alignment for L>=20', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(2*10**1,10**3), fontsize=15, xytext=(25,10**3.7))
...
plt.savefig('/bucket/.mabuya/MillerU/Abrar/PCS/hg38-rheMac10/hgrm.png')
#ax.show()