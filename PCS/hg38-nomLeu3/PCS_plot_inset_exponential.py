...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-nomLeu3/PCS_hg38nomLeu3.csv") 
df = df[df['Number of PCS of Length L']!=0][9:]
# Preparing the data for the plot
x = [i for i in df.index][:]
y = df['Number of PCS of Length L'][:]
x_fake = [i for i in df.index][:730]
print(x_fake[0],x_fake[-1])
y_fake = df['Number of PCS of Length L'][:730]#29:790
...
axIns = ax.inset_axes([0.055, 0.055, 0.49, 0.49])
...
ax.set_title('PCS length distribution of hg38/nomLeu3 genome alignment for L>=12', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(15,10**3.7), fontsize=15, xytext=(15,10**3.7))
...
plt.savefig('/bucket/.mabuya/MillerU/Abrar/PCS/hg38-nomLeu3/hgnl3.png')
#ax.show()