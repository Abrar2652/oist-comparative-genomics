...
genome_list = ['colAng1','rhiBie1','rhiRox1']
for genome in genome_list:
    ...
    def compare_strings1(df, column, j):
        ...
        for i in range(maxlen):
          #use a slice rather than index in case one string longer than other
            ...
            if (letter1.islower() or letter2.islower()):
                result1+='\n'
                result2+='\n'
            if ((letter1 not in ['A','T','C','G']) or (letter2 not in ['A','T','C','G'])): # smallercase terminate
                result1+='\n'
                result2+='\n'
            if (letter1.isupper() and letter2.isupper() and (letter1!='-') and (letter2!='-') and 
    ...
    def matched_indices(df, column, j):
        ...
        for i,(letter1, letter2) in enumerate(zip(str1,str2)):#i=index, letter1=str1, letter2=str2
            #if ((letter1 == letter2) and (letter1 in ['A','T','C','G']) and (letter2 in ['A','T','C','G'])): # mismatch_terminated
    ...
    #header_frame.to_csv(r'hg38_{1}_{0}_vanilla_indel_terminated_seqs.tsv'.format(chrom, genome), sep="\t", index=False)
    ...
    df.to_csv(r'vanilla_indel_terminated_PCS_hg38{0}.csv'.format(genome), index = False) 
