...
import pandas as pd
import numpy as np
new = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/Uppercase_analysis/indel_terminated/hg38_{0}_all_vanilla_indel_terminated_seqs.tsv".format(genome),sep='\t')
new = new.sort_values('pcs.width',ascending=False)
df = new[['first.sequence','second.sequence']][:cutoff]
lines = []
    ...
    for i in range(maxlen):
      #use a slice rather than index in case one string longer than other
        ...
        if (letter1!=letter2 and letter1 in ['A','T','C','G','-'] and letter2 in ['A','T','C','G','-']):
            result1+=letter1
            result2+=letter2
        if ((letter1 == letter2)): # match terminate
            result1+='\n'
            result2+='\n'
...
#cum_pcs_frame.to_csv(r'/bucket/MillerU/Abrar/PCS/Uppercase_analysis/cumulative_CMMS_pcs_for_each_IGS_whole_genome/cumulative_CMMS_pcs_in_vanilla_IGS_hg38{0}_largest{1}.csv'.format(genome,cutoff), index = False) 
...
with plt.rc_context({"axes.prop_cycle": get_cycle('gist_rainbow',cutoff)}):
  for i,k in enumerate(cum_pcs_per_igs):
...
plt.title('Cumulative Contiguous Mismatched PCS Length Distribution\n Comparison For Each Vanilla IGS in hg38/{0}'.format(genome), fontsize=14)
...
plt.savefig('/bucket/MillerU/Abrar/PCS/Uppercase_analysis/cumulative_CMMS_pcs_for_each_IGS_whole_genome/cumulative_CMMS_pcs_in_vanilla_IGS_hg38{0}_largest{1}.png'.format(genome,cutoff))
