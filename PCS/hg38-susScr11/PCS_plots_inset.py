...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-susScr11/PCS_hg38susScr11.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][:]
x_fake = [i for i in df.index][9:490]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][:]
y_fake = df['Number of PCS of Length L'][9:490]
x=np.array(x)
...
y_fit = linear(x_fake, 3.9, 10**10.5)
...
ax.set_title('PCS length distribution of hg38/susScr11 genome alignment for L>=1', fontsize=15)
ax.annotate("MSE: {:.3f}\nRMSE: {:.3f}\nR-squared: {:.3f}".format(MSE,RMSE,Rsquared),(2,10**5), fontsize=15, xytext=(2,10**5))
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-susScr11/hgsusScr11.png")
#plt.show()
