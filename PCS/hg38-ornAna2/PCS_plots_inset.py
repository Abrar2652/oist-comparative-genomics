...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-ornAna2/PCS_hg38ornAna2.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][:]
x_fake = [i for i in df.index][6:330]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][:]
y_fake = df['Number of PCS of Length L'][6:330]
x=np.array(x)
...
ax.set_title('PCS length distribution of hg38/ornAna2 genome alignment for L>=1', fontsize=15)
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-ornAna2/hgornAna2.png")
#plt.show()
