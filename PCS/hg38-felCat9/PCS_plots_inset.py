...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-felCat9/PCS_hg38felCat9.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][:]
x_fake = [i for i in df.index][9:500]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][:]
y_fake = df['Number of PCS of Length L'][9:500]
x=np.array(x)
...
y_fit = linear(x_fake, 3.8, 10**10.4)
...
ax.set_title('PCS length distribution of hg38/felCat9 genome alignment for L>=1', fontsize=15)
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-felCat9/hgfelCat9.png")
#plt.show()
