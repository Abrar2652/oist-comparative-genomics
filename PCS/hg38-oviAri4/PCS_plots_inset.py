...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-oviAri4/PCS_hg38oviAri4.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][:]
x_fake = [i for i in df.index][10:450]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][:]
y_fake = df['Number of PCS of Length L'][10:450]
x=np.array(x)
...
ax.set_title('PCS length distribution of hg38/oviAri4 genome alignment for L>=1', fontsize=15)
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-oviAri4/hgoviAri4.png")
#plt.show()
