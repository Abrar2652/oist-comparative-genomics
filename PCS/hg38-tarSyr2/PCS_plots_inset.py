...
df = pd.read_csv("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-tarSyr2/PCS_hg38tarSyr2.csv") 
df = df[df['Number of PCS of Length L']!=0]
# Preparing the data for the plot
x = [i for i in df.index][:]
x_fake = [i for i in df.index][13:520]

print(x_fake[0], x_fake[-1])
y = df['Number of PCS of Length L'][:]
y_fake = df['Number of PCS of Length L'][13:520]
x=np.array(x)
...
ax.set_title('PCS length distribution of hg38/tarSyr2 genome alignment for L>=1', fontsize=15)
...
plt.savefig("/bucket/.mabuya/MillerU/Abrar/PCS/hg38-tarSyr2/hgtarSyr2.png")
#plt.show()
