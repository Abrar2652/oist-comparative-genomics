...
#nash_kurtosis = pd.read_csv("/bucket/MillerU/Abrar/Moment_val_bins/hg38nomLeu3/All_bin_30kb/kurtosis_100pc_500col.csv") #in the Moment_val_bins folder, the kurtosis files are located
nash_kurtosis = pd.read_csv("/bucket/MillerU/Abrar/Moment_val_bins/hg38nomLeu3/All_bin_15kb/quantile_kurtosis_100pc.csv") 
...
st2 = 'PCS distribution (with exons) having Heavy Tail (left hand side cluster) in Quantile Kurtosis'
species1 = 'hg38'
species2 = 'nomLeu3'
...
df.to_csv(r'/bucket/MillerU/Abrar/PCS_heavy_tailed/high_kurtosis_PCS_counts_hg38nomLeu3.csv', index = False) 
...
x = [x for x in range(int(max(final_tail_pcs)+5))][12:]
y = serial_count[12:]
