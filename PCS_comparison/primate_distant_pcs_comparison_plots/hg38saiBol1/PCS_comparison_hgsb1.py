...
import pandas as pd
# kurtosis_100pc_500col.csv" in case of saiBol1
nash_kurtosis = pd.read_csv("/bucket/MillerU/Abrar/Moment_val_bins/hg38saiBol1/All_bin_15kb/quantile_kurtosis_100pc.csv") #in the Moment_val_bins folder, the kurtosis files are located
...
dfx = pd.read_csv("/bucket/MillerU/Abrar/PCS/hg38-saiBol1/PCS_hg38saiBol1.csv")

x = [i for i in dfx.index][10:]
y = dfx['Number of PCS of Length L'][10:]
m = [x for x in range(int(max(final_tail_pcs1)+5))][10:]
n = serial_count1[10:]
q = [x for x in range(int(max(final_tail_pcs2)+5))][10:] 
r = serial_count2[10:]
a = [x for x in range(int(max(final_tail_pcs3)+5))][10:] 
b = serial_count3[10:]
c = [x for x in range(int(max(final_tail_pcs4)+5))][10:] 
d = serial_count4[10:]
s = [x for x in range(int(max(final_tail_pcs5)+5))][10:] 
t = serial_count5[10:]
...
plt.plot(x, y, '.r', linewidth=2, label='Human/Squirrel Monkey whole genome PCS (with exons)')
...
plt.savefig("/bucket/MillerU/Abrar/PCS_comparison/hg38saiBol1/PCS_comparison_hg38saiBol1_linear.png")
...
# Plotting the graph with Log ticks at x and y axis using loglog
ax.loglog(x, y, '.r', linewidth=2, label='Human/Squirrel Monkey whole genome PCS (with exons)')
...
plt.savefig('/bucket/MillerU/Abrar/PCS_comparison/hg38saiBol1/PCS_comparison_hg38saiBol1_loglog.png')
...
plt.semilogy(x, y, '.r', linewidth=2, base=10, label='Human/Squirrel Monkey whole genome PCS (with exons)')
...
plt.title("Heavy-tailed vanilla PCS length distribution comparisons of\n hg38/saiBol1 genome alignment (semi-log) for L>=10", fontsize=14)
plt.xlabel('PCS Length', fontsize=20)
...
plt.legend(fontsize=13, markerscale=3)
plt.savefig('/bucket/MillerU/Abrar/PCS_comparison/hg38saiBol1/PCS_comparison_hg38saiBol1_semilog.png')
